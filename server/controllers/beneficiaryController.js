...
const newBeneficiary=async(req,res)=>{
   ...
   res.status(400).json({error:error.message})
    ...
    const allBeneficiarys = (req, res) => { 
           // and operation allows me to join conditions 
      ...
      // donation cards displayed in the user profile
    const allUserCards = (req, res) => {
        const usersId = req.params.id;

        Beneficiarys.find({ $and : [ { usersId : { $in: usersId}}, { $expr: { $ne: ['$price', '$currentDonation']}}]})
        .then ((data)=> {
            res.json(data);
        })
        .catch((error)=> {
      ...
      // donation cards displayed in the user profile for cases that already reached the amount needed
    ...
    const allUserCardsF = (req, res) => { 
        ...
        const updateBeneficiary= async (req, res) => {
                ...
                const beneficiary = await Beneficiarys.findByIdAndUpdate(beneficiaryId, updatedBeneficiaryData, { new: true }); //this 3rd (new) argument tells mongoose to return the modified document  
