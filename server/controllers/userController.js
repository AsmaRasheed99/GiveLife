...
const newUser = async (req, res) => {
    ...
    if (user0.length == 0){
        ...
        console.log(addUser)
        const token = jwt.sign({id: addUser._id, username: addUser.firstName ,role : addUser.role})
...
const newUserLogin = async (req, res) => {
  ...
  if (user.length != 0) {
    ...
    if(validpassword) {
     const token = jwt.sign({id: user[0]._id, username: user[0].firstName ,role: user[0].role, message:user[0].message, email:email}, SECRETKEY, { expiresIn: '3h'});
     const user0 =user[0]
...
const protected = async (req , res) => {
  const token =req.headers.authorization.trim(); //remove spaces//
  if (!token) {
